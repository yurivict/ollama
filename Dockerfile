...
RUN --mount=type=cache,target=/root/.ccache \
    ...
    && cmake -S . -B build/mlx_cuda_v13 -DOLLAMA_MLX_BACKENDS=cuda_v13 -DBLAS_INCLUDE_DIRS=/usr/include/openblas -DLAPACK_INCLUDE_DIRS=/usr/include/openblas -DCMAKE_CUDA_FLAGS="-t ${OLLAMA_MLX_NVCC_THREADS}" ${MLX_CUDA_RAM_MB:+-DMLX_CUDA_RAM_MB=${MLX_CUDA_RAM_MB}} -DOLLAMA_PAYLOAD_INSTALL_PREFIX=/go/src/github.com/yurivict/ollama/dist \
...
COPY --from=build /go/src/github.com/yurivict/ollama/build/go-license/lib/ollama/GO_LICENSE /lib/ollama/GO_LICENSE
