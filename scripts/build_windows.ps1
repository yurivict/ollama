...
function buildOllamaCLI {
    ...
    & go build -trimpath -ldflags "-s -w -X=github.com/yurivict/ollama/version.Version=$script:VERSION -X=github.com/yurivict/ollama/server.mode=release" -o "${distDir}\ollama.exe" .
    if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
}
...
function buildApp {
	...
	& go build -trimpath -ldflags "-s -w -H windowsgui -X=github.com/yurivict/ollama/app/version.Version=$script:VERSION" -o .\dist\windows-ollama-app-${arch}.exe ./app/cmd/app/
    if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
}
